...
func buildChannelListQuery(group string, statusFilter int, typeFilter int, source ...string) *gorm.DB {
	query := model.DB.Model(&model.Channel{})
	if len(source) > 0 && source[0] == "api" {
		query = query.Where("type != ?", constant.ChannelTypeClientOAuth)
	}
...
func GetAllChannels(c *gin.Context) {
	...
	if enableTagMode {
		tags, err := model.GetPaginatedChannelTags(buildChannelListQuery(groupFilter, statusFilter, typeFilter, c.Query("source")), pageInfo.GetStartIdx(), pageInfo.GetPageSize())
		if err != nil {
			common.SysError("failed to get paginated tags: " + err.Error())
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "获取标签失败，请稍后重试"})
			return
		}
		total, err = model.CountChannelTags(buildChannelListQuery(groupFilter, statusFilter, typeFilter, c.Query("source")))
		if err != nil {
		...
		for _, tag := range tags {
			...
			err := sortOptions.Apply(buildChannelListQuery(groupFilter, statusFilter, typeFilter, c.Query("source")).Where("tag = ?", *tag)).
				Omit("key").
		...
		if err := buildChannelListQuery(groupFilter, statusFilter, typeFilter, c.Query("source")).Count(&total).Error; err != nil {
			common.SysError("failed to count channels: " + err.Error())
			c.JSON(http.StatusOK, gin.H{"success": false, "message": "获取渠道数量失败，请稍后重试"})
			return
		}

		err := sortOptions.Apply(buildChannelListQuery(groupFilter, statusFilter, typeFilter, c.Query("source"))).
	...
	countQuery := buildChannelListQuery(groupFilter, statusFilter, -1, c.Query("source"))
	var results []struct {
		Type  int64
		Count int64
	}
...
func SearchChannels(c *gin.Context) {
	...
	if enableTagMode {
		...
		for _, tag := range tags {
			if tag != nil && *tag != "" {
				var tagChannels []*model.Channel
				err := sortOptions.Apply(buildChannelListQuery(group, -1, -1, c.Query("source")).Where("tag = ?", *tag)).
					Omit("key").
					Find(&tagChannels).Error
				if err != nil {
					c.JSON(http.StatusOK, gin.H{
		...
		if c.Query("source") == "api" {
			filtered := make([]*model.Channel, 0, len(channelData))
			for _, item := range channelData {
				if item.Type != constant.ChannelTypeClientOAuth {
